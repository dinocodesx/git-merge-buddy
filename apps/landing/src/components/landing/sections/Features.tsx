...
import { ArrowRight, Circle, Zap, ShieldAlert, Cpu, FileJson } from 'lucide-react';
    ...
    <div className="max-w-7xl mx-auto">
      ...
      <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
        <FeatureCard 
          ...
          description="1-click commits for easy fixes and a 'Fix with AI' button for more complex architectural refactors."
          content={
            <div className="bg-zinc-950 p-4 border-2 border-primary/20 font-mono text-[10px] md:text-xs text-zinc-300">
              <div className="flex justify-between items-center mb-4 border-b border-primary/10 pb-2">
                ...
                <span className="bg-primary text-black px-2 text-[10px] font-bold">STABLE FIX</span>
              </div>
              <div className="space-y-1">
                <div className="text-red-400 bg-red-950/30 line-through opacity-60">- async function fetchAll(ids: string[]) &#123;</div>
                <div className="text-red-400 bg-red-950/30 line-through opacity-60">-   return ids.map(async id =&gt; await db.get(id));</div>
                <div className="text-red-400 bg-red-950/30 line-through opacity-60">- &#125;</div>
                <div className="text-green-400 bg-green-950/30">+ async function fetchAll(ids: string[]) &#123;</div>
                <div className="text-green-400 bg-green-950/30">+   return Promise.all(ids.map(id =&gt; db.get(id)));</div>
                <div className="text-green-400 bg-green-950/30">+ &#125;</div>
              </div>
              <div className="mt-4 flex gap-2">
                <div className="bg-primary text-black px-3 py-1 font-bold text-[10px] uppercase cursor-pointer">Apply Fix</div>
                <div className="border border-white/20 text-white px-3 py-1 text-[10px] uppercase cursor-pointer">Review</div>
              </div>
            </div>
          }
        />
        <FeatureCard 
          ...
          description="Quick context with a summary of changes, a walkthrough & an architectural diagram."
          content={
            <div className="h-full flex flex-col justify-center items-center bg-zinc-950/50 p-6 border-2 border-primary/20 space-y-4">
               ...
               <div className="text-[10px] text-zinc-500 font-mono italic">MAP: AuthProxy -&gt; TokenStore</div>
            </div>
          }
          className=""
        />
        <FeatureCard 
          ...
          description="We find bugs humans miss – & flag the time consuming and tedious. Without the noise."
          content={
            <div className="bg-zinc-950 p-4 border-2 border-primary/20 font-mono text-xs h-full flex flex-col justify-center gap-2">
              <div className="bg-zinc-900 p-3 border-l-4 border-red-500">
                <div className="text-red-500 font-bold mb-1 uppercase text-[10px]">Critical: Race Condition</div>
                <div className="text-zinc-400 text-[10px] leading-tight">
                  Updating <code className="text-zinc-200">counter</code> without a transaction could lead to inconsistent state...
                </div>
              </div>
              <div className="bg-zinc-900 p-3 border-l-4 border-yellow-500">
                <div className="text-yellow-500 font-bold mb-1 uppercase text-[10px]">Warning: Re-entrancy</div>
                <div className="text-zinc-400 text-[10px] leading-tight">
                  External call before state update in <code className="text-zinc-200">withdraw()</code>...
                </div>
              </div>
            </div>
          }
        ...
        />
         <FeatureCard 
          ...
          description="Give feedback on reviews to create Learnings. Or create issues, trigger docstrings & more."
          content={
            <div className="bg-zinc-950 p-4 border-2 border-primary/20 font-mono text-xs space-y-4">
              <div className="flex gap-2">
                ...
                <div className="bg-zinc-900 p-2 rounded-lg text-white">
                  <div className="text-[10px] text-zinc-400 mb-1">jennjenn <span className="opacity-50">(author)</span></div>
                  <div className="text-[10px] uppercase">@gitmergebuddy we want to get rid of the star imports</div>
                </div>
              </div>
              <div className="flex gap-2 justify-end">
                <div className="bg-primary/10 border border-primary/30 p-2 rounded-lg text-right">
                  <div className="text-[10px] text-primary mb-1">Git Merge Buddy <span className="opacity-50">bot</span></div>
                  <div className="text-zinc-200 text-[10px]">UNDERSTOOD. CONVERTING STAR IMPORTS TO NAMED EXPORTS...</div>
                </div>
        ...
        <FeatureCard 
          ...
          description="Customize everything from your coding guidelines to your workflow in a yaml file."
          content={
            <div className="bg-zinc-950 p-4 border-2 border-primary/20 font-mono text-[10px] h-full flex flex-col justify-center items-center">
              <div className="flex items-center gap-3 bg-zinc-900 p-4 border border-primary/20">
                 ...
                 <div className="font-black text-xl text-primary tracking-tighter">YAML</div>
              </div>
              <div className="mt-4 text-zinc-500 italic uppercase tracking-widest text-[8px]">ENGINE_CONFIG.YAML</div>
            </div>
          }
          className=""
        />
        <FeatureCard 
          ...
          description="Automate the creation of your daily standup reports, sprint reviews, and more."
          content={
            <div className="bg-zinc-950 p-6 border-2 border-primary/20 font-space text-white h-full flex flex-col justify-center">
               <div className="space-y-4">
                  <div className="text-[10px] text-zinc-500 uppercase tracking-widest font-bold">Quality Focus</div>
                  <div className="space-y-1">
